...
void ev001A_s_eggmantakesbirdie(int state)
{
	...
	case 1:
		...
		EV_SetPos(player, 6.3000002f, 1525.7f, 3041.0f); //Hide Sonic
		...
		EV_SetPos(amy, -55.599998f, 1525.67f, 3320.0f);
		EV_SetAng(amy, 0, 0x8000, 0);
		WING = SetEventBirdie0();
		EV_SetPos(WING, 2.8199999f, 1638.08f, 3195.52f);
		EV_SetAng(WING, 0, -0x8000, 0);
		EV_ClrAction(amy);
		EV_SetAction(amy, &action_a_a0127_amy, &AMY_TEXLIST, 2.0f, 1, 0);
		EV_SetMode(WING, 0);
		EV_SetAction(WING, &action_w_w0127_wing, &VER2_WING_TEXLIST, 2.0f, 1, 0);
		...
		EV_Wait(15);
		EV_MovePoint2(amy, -55.599998f, 1525.67f, 3273.1899f, 2.0f, 0.059999999f);
		moveObjectOn(WING, 0.0f, 0.0f, 0.0f, 900, amy);
		EV_Wait(70);
		EV_SerifPlay(922);
		EV_SetFace(amy, "HB");
		...
		stopObject(WING);
		EV_SetAng(WING, 0, 0x8000, 0);
		EV_SetAction(WING, &action_w_w0835_wing, &VER2_WING_TEXLIST, 1.0f, 1, 8);
		EV_SetAction(amy, &action_a_a0835_amy, &AMY_TEXLIST, 1.0f, 1, 8);
		EV_Wait(10);
		EV_CameraPos(1, 70, -65.589996f, 1529.5601f, 3265.0f);
		EV_CameraAng(1, 70, 0x20F, 0xB257, 0);
		EV_Wait(30);
		EV_LookPoint(amy, -64.400002f, 1532.0f, 3266.0f);
		EV_Wait(10);
		EV_LookPoint(amy, -21.0f, 1532.0f, 3293.0f);
		EV_Wait(10);
		EV_MsgClose( );
		EV_ClrAction(amy);
		EV_LookFree(amy);
		EV_ClrAction(WING);
		EV_SetAction(amy, &action_a_a0814_amy, &AMY_TEXLIST, 1.0f, 1, 0);
		EV_SetAction(WING, &action_w_w0814_wing, &VER2_WING_TEXLIST, 1.0f, 1, 0);
		EV_SetAng(amy, 0, 0x5878, 0);
		EV_SetAng(WING, 0, 0x5878, 0);
		...
		EV_Wait(30);
		EV_ClrAction(amy);
		EV_SetAction(amy, &action_a_a0126_amy, &AMY_TEXLIST, 1.0f, 1, 0);
		moveObjectOn(WING, 0.0f, 0.0f, 0.0f, 900, amy);
		moveObject(amy, -55.599998f, 1525.6875f, 3273.657f, -65.889999f, 1526.0f, 3280.3601f, 60);
		EV_Wait(25);
		EV_SetAction(amy, &action_a_a0814_amy, &AMY_TEXLIST, 1.0f, 1, 4);
		EV_Wait(10);
		stopObjectAll( );
		EV_SetPos(amy, -55.599998f, 1525.6875f, 3273.657f);
		EV_SetPos(WING, -55.599998f, 1525.6875f, 3273.657f);
		...
		EV_LookFree(amy);
		EV_LookPoint(amy, -9.6000004f, 1550.0f, 3241.01f);
		EV_SetFace(amy, "EE");
		EV_SerifPlay(923);
		...
		EV_SetPos(eggmoble, 22.799999f, 1535.0f, 3231.6001f);
		EV_SetAng(eggmoble, 0, 0x3800, 0);
		EV_SetPos(amy, -15.0f, 1525.687f, 3245.5f);
		EV_SetAng(amy, 0, 20448, 0);
		EV_SetPos(WING,
			amy->twp->pos.x,
			amy->twp->pos.y,
			amy->twp->pos.z);
