...
void ev0008_s_chaos1(int state)
{
	...
	case 1:
		...
		EV_Msg(msgTbl_ev0080[ TextLanguage ][ 9 ]); // Oh no!
		EV_SerifPlay(444);	
		EV_SetAction(player, &action_s_s0009_sonic, &SONIC_TEXLIST, 0.40000001f, 1, 4);
		stopObject(C_EME_P);
		...
		EV_SetAng(tails, 0x18, 0x3400, 0x3E);
		...
		EV_LookPoint(tails, 1096.2f, 126.3f, 864.79999f);
		EV_SerifPlay(447);
		...
		EV_ClrFace(tails);
		EV_SetFace(tails, "ABAAAAAAAABAAAAABAAAAAAB");
		EV_SerifPlay(655);
		EV_SetFace(tails, "AB");
		EV_Msg(msgTbl_ev0034[ TextLanguage ][ 6 ]); //"\aUh, Monster?"
		if ( VoiceLanguage == Languages_Japanese )
		{
			EV_SetFace(tails, "BBABB");
		}
		if ( VoiceLanguage == Languages_English )
		{
			EV_SetFace(tails, "CCCCC");
		}
		EV_Wait(40);
		EV_ClrFace(tails);
		EV_MsgClose( );
