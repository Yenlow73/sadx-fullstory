...
void ev0039_t_beforechaos4(int state)
{
	...
	case 1:
		...
		EV_SetPos(player, 37.299999f, 72.019997f, 313.32001f);
		EV_SetAng(player, 0, 0x7900, 0);

		EV_CreatePlayer(2, KnucklesTheEchidna, 73.0f, 73.050003f, 380.60001f, 0xFF96, 0x8000, 18);
		EV_CreatePlayer(3, SonicTheHedgehog, 73.0f, 72.0f, 170.0f, 0xF924, 0, 0);
		EV_Wait(1);
		...
		ChgEggMobleMod(5);
		EV_CreateObject(&B_EME, sonic->twp->pos.x, sonic->twp->pos.y, sonic->twp->pos.z, 0, 0, 0);
		EV_CreateObject(&G_EME, sonic->twp->pos.x, sonic->twp->pos.y, sonic->twp->pos.z, 0, 0, 0);
		...
		moveObject(sonic, sonic->twp->pos.x, sonic->twp->pos.y, sonic->twp->pos.z, sonic->twp->pos.x, 72.349998f, sonic->twp->pos.z + 48.0f, 15);
		EV_ClrAction(knuckles);
		...
		EV_Wait(10);
		dsPlay_oneshot_v(1338, 0, 0, 120, sonic->twp->pos.x, 72.349998f, sonic->twp->pos.z + 48.0f);
		EV_Wait(5);
		...
		moveObject(sonic, sonic->twp->pos.x, sonic->twp->pos.y, sonic->twp->pos.z, 73.0f, 72.0f, 230.0f, 30);
		moveObject(knuckles, knuckles->twp->pos.x, knuckles->twp->pos.y, knuckles->twp->pos.z, 73.0f, 72.0f, 320.60001f, 30);
		moveObject(SMOKE_01, sonic->twp->pos.x, sonic->twp->pos.y, sonic->twp->pos.z, 73.0f, 72.0f, 230.0f, 35);
		...
		EV_SetAng(sonic, sonic->twp->ang.x, -0x4000 - sonic->twp->ang.y, sonic->twp->ang.z);

		EV_LookFree(player);
		EV_ClrAction(player);
		EV_SetAction(player, &action_m_m0001_miles, &MILES_TEXLIST, 1.0f, 1, 0);
		EV_SetPos(player, 73.0f, 72.019997f, 232.51f);
